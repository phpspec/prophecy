...
class MethodProphecy extends ObjectBehavior
{
    /**
     ...
     */
    function it_records_promise_through_will_method($promise)
    {
        $this->will($promise);
    ...
    function it_adds_ReturnPromise_during_willReturn_call()
    {
        $this->willReturn(42);
    ...
    function it_adds_ReturnArgumentPromise_during_willReturnArgument_call()
    {
        $this->willReturnArgument();
        $this->getPromise()->shouldBeAnInstanceOf('Prophecy\Promise\ReturnArgumentPromise');
    }

    function it_adds_CallbackPromise_during_will_call_with_callback_argument()
    {
        $callback = function(){};
     ...
     */
    function it_records_prediction_through_should_method($prediction)
    {
        $this->callOnProphetSubject('should', array($prediction));
        $this->getPrediction()->shouldReturn($prediction);
    }

    function it_adds_CallbackPrediction_during_should_call_with_callback_argument()
    {
        $callback = function(){};

        $this->callOnProphetSubject('should', array($callback));
    ...
    function it_adds_CallPrediction_during_shouldBeCalled_call()
    {
        $this->callOnProphetSubject('shouldBeCalled', array());
        $this->getPrediction()->shouldBeAnInstanceOf('Prophecy\Prediction\CallPrediction');
    }

    function it_adds_NoCallsPrediction_during_shouldNotBeCalled_call()
    {
        $this->callOnProphetSubject('shouldNotBeCalled', array());
        $this->getPrediction()->shouldBeAnInstanceOf('Prophecy\Prediction\NoCallsPrediction');
    }

    function it_adds_CallTimesPrediction_during_shouldBeCalledTimes_call()
    {
        $this->callOnProphetSubject('shouldBeCalledTimes', array(5));
    ...
    )
    {
        ...
        $this->callOnProphetSubject('shouldHave', array($prediction));
    }
